...
const addFriend = async (req, res, next) => {
  try {
    ...
    const alreadyFriend = req.user.friends.map(f => f.toString()).includes(id);
    if (alreadyFriend) {
...
const removeFriend = async (req, res, next) => {
  try {
    ...
    target.friends = target.friends.filter(f => f.toString() !== req.user._id.toString());
...
const getFriendProfile = async (req, res, next) => {
  try {
    ...
    res.json({ friend, sharedChallenges: sharedFormatted });
  } catch (err) {
    next(err);
  }
};
