<p align="center"><img src="https://www.unir.net/wp-content/uploads/2019/11/Unir_2021_logo.svg" alt="Logo_UNIR" /></p>
<p align="center" style="font-family: 'Calibri Light'; font-size:30px">Universidad Internacional de La Rioja</p>
<p align="center" style="font-family: 'Calibri Light'; font-size:26px">Escuela Superior de Ingeniería y Tecnología</center></p>

<p align="center" style="font-family: 'Calibri Light'; font-size:24px">Grado en Ingeniería Informática</p>
<p align="center" style="font-family: 'Calibri'; font-size:30px; color:#0098CD">Applicacion hibrida en Android para desafio de pasos con Google Fit</p>

<div style="display: table; margin: 0 auto;" align="center">
  <table>
    <tr style="background-color:white">
      <td>Trabajo fin de grado presentado por:</td>
      <td>Borja Llorente Capiscol</td>
    </tr>
    <tr style="background-color:white">
      <td>Director:</td>
      <td>Jaime Rodrigo Segarra Escandon</td>
    </tr>
    <tr style="background-color:white">
      <td>Fecha:</td>
      <td>TBD</td>
    </tr>
    <tr style="background-color:white">
      <td>Repositorio del código fuente:</td>
      <td><a href="https://github.com/b0rj1t4/TFG_Foozy" target="_blank">TFG</a></td>
    </tr>
  </table>
</div>

<p style="font-family: 'Calibri'; font-size:20px; margin: 0 auto;" align="center"><strong>CÓDIGO</strong></p>

<div align="center">
  	<a href="https://github.com/b0rj1t4/TFG_Foozy/tree/main/BACK" target="_blank"><img src="https://img.shields.io/badge/Backend-V0.0.0-512BD5?logo=Node.js&logoColor=white"></a>
  	<a href="https://github.com/b0rj1t4/TFG_Foozy/tree/main/UI" target="_blank"><img src="https://img.shields.io/badge/Frontend-V0.0.1-09D9FD?logo=Ionic&logoColor=white"></a>
</div>
